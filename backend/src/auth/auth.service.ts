...
export class AuthService {
    ...
    // Generated server-side after OAuth and exchanged by the mobile app.
    // This avoids passing the JWT in the redirect URL (which would appear in logs/history).
    //
    // B-4 ⚠️  SINGLE-INSTANCE ONLY: this Map lives in process memory.
    // In a horizontally-scaled deployment (multiple dynos / PM2 cluster / Docker replicas)
    // the code is generated by one instance but the /auth/token request may hit a
    // different instance that has no record of it → exchange fails.
    // Fix before scaling: replace with a Redis SETEX with a 60-second TTL.
